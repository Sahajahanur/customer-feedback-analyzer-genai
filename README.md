...
---
Make it your own (your turn)
The project works, but a real project is never "finished." Try these:
Show a chart. Use `st.bar_chart` to show how many reviews fall under each theme. (Hint: `collections.Counter` already counts them.)
Filter the table. Add a dropdown (`st.selectbox`) to show only "negative" reviews, so the owner can focus on problems first.
Read from a file. Add a button that loads `sample_reviews.txt` straight into the text box, instead of pasting by hand (Lesson 8).
New backend field. Ask Gemini for a one-line `suggestion` on how the business could fix each negative review. (Watch your tokens — keep it short!)
---
You did it
